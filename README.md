...
- **備料員盤點情況 (人員績效卡片與分頁機制)**：
  - 獨立列出每位盤點人員的卡片。卡片內建**分頁 (Tabs) 切換功能**，分為「📊 統計數據」與「📋 未完成」兩大分頁。
  - **統計數據**：顯示該員的「本月派送」、「本月完成」以及「完成率」。
  - **未完成清單**：列出該員所有處理中的單據，並且內建**每頁 5 筆的翻頁功能**，避免資料過長破壞版面。清單上方會統計「總計單據」與「總計項目數」。未完成分頁的右上角更配有**醒目放大、加粗的紅色數字徽章**，強烈提示待辦數量。
  - 設有「🌟 全員總計」看板，方便查看整個團隊的總和表現（包含全員的總盤點項目數）。
  - **範例**：管理者在月初登入時，可透過下拉選單切換至「本年度/本月份」，迅速看出小明這個月已完成 5 件盤點。點擊小明卡片上的「未完成」分頁，可翻頁瀏覽他手上剩餘的單據詳情。
- **近六個月盤點趨勢圖**：
  - 以圖表呈現過去半年每月的盤點業務量。
  - 支援下拉選單自由切換要顯示的圖表類型（長條圖、折線圖、二者並存）以及數據（盤點數量、盤點項目數量、全部顯示）。
...
- **分頁與顯示設定**：清單上方與下方皆配有翻頁按鈕。在條件摘要列的右側，管理者可透過下拉選單即時調整「每頁筆數」（5/10/15/20 筆），版面配置緊湊且直覺。
...
將盤點作業指派給特定人員。
- **單筆派送**：點擊人員專屬的「個人卡片」上的「派送」按鈕（畫面內建下拉選單可綁定特定盤點任務），輸入盤點單號、類型（夾鉗/TKW）、項目數量即可完成派發。
- **批次派送**：支援將一段連續的單號（例如 260001 到 260050）一次性派發給某位人員，系統會自動產出 50 張盤點單。當選擇批次派送時，畫面會貼心提示並隱藏不適用的「項目數量」欄位。
- **範例**：主管收到 10 張新工單，直接在派送頁面找到「小強」的卡片，點擊「派送」，輸入單號並選擇了「第一季大盤點」任務，即可將工作交給他。卡片上也會即時顯示小強目前還有幾件未完成的單據，避免勞逸不均。

### 5. 📈 統計作業 (Statistics)
進階的數據分析報表，幫助管理層進行效能檢討。
- **自訂過濾條件與任務篩選**：可輸入「日期區間」、「盤點單號區間」或是指定某個「盤點任務」來產生特定範圍的報表。如果文字可能被裁切，版面會自動換行以維持美觀。
- **人員績效圖表**：支援直條圖與圓餅圖，可切換檢視每位人員的「總盤點數量」、「完成率」或「平均處理天數」。人員總計區塊內也會詳列「總盤點項目數」。
- **各流程平均處理天數**：統計並用圖表展示每個階段（如：初盤、收料）平均卡關的天數，幫助找出瓶頸。
- **範例**：主管想知道「2026年第一季」大家的效率，便輸入日期區間並選定任務，系統立即算出團隊總計盤點數，並畫出各流程的平均天數長條圖，主管一看發現「複盤」平均耗時高達 5 天，便能著手進行流程改善。
...
維護系統的權限與人員名單。
- **人員設定與職責過濾**：可設定姓名、職稱、工作職責（如：備料、盤點、生管等）與備註。列表上方提供「工作職責」的下拉篩選器，方便快速尋找特定角色。
- **手繪性別圖示與顯眼標籤**：捨棄傳統 Emoji，採用純手工繪製、更大更清晰的 SVG 性別符號（♂ 帶箭頭的男 / ♀ 帶十字的女 / ⚥ 其他）。個人卡片上的人員姓名採用極粗字體（`900` 粗體、`2rem` 大小），視覺層級分明。
- **操作友善的表單**：新增與編輯表單右上角配有明顯的塗鴉風格「❌」關閉按鈕，操作更直覺。
- **範例**：新人小陳報到，管理員至此頁面點擊「新增人員」，輸入小陳的基本資料並勾選「盤點」與「備料」職責，小陳專屬的塗鴉卡片便會建立完成。日後若只需檢視「備料」人員，只要在上方下拉選單選擇「備料」即可。
